...
classify_note = capture_agent.classify_note
create_classification_agent = capture_agent.create_classification_agent
...
async def test_direct_llm(text: str) -> tuple[Dict, float]:
...
async def test_react_agent(text: str) -> tuple[Dict, float]:
...
async def run_comparison():
    ...
    for i, scenario in enumerate(TEST_SCENARIOS, 1):
        ...
        try:
            direct_result, direct_time = await test_direct_llm(scenario["text"])
        ...
        try:
            react_result, react_time = await test_react_agent(scenario["text"])
    ...
    output_file = Path(__file__).parent / "classification_comparison_results.json"
