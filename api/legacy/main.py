...
from .models import (
    ...
    CaptureNoteResponse, EpisodicMetadata, TimeReference  # GraphRAG models
)
...
@app.post("/capture_note", response_model=ClassifyResponse)
async def capture_note(req: ClassifyRequest):
    ...
    4. Create prospective edges for time references (Phase 3)
    5. Background: Semantic linking via embeddings (Phase 2 - TODO)
    """
    ...
    try:
        ...
        episodic = await extract_episodic_metadata(req.text, current_date)
        ...
        store_graph_node(
            ...
            text=req.text,
            file_path=filepath,
            episodic_metadata=episodic,
            db_connection=con
        )

        # Phase 3: Prospective layer - Create time-based edges (simple version)
        # TODO: Implement time_next edge creation for future references
        # For now, just store time references in node metadata (already done above)

        # Commit transaction
        con.commit()

        # Return response (dimensions deprecated, set to empty)
        return ClassifyResponse(
            title=title,
            dimensions=DimensionFlags(),  # GraphRAG doesn't use dimension flags
            tags=episodic["tags"],
            path=filepath
        )
...
async def classify_and_save(req: ClassifyRequest):
    """Legacy endpoint - redirects to /capture_note"""
    return await capture_note(req)
